...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"nlme"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(}\StringTok{"piecewiseSEM"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{lake <{-}}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"./Data/Raw/NTL{-}LTER\_Lake\_ChemistryPhysics\_Raw.csv"}\NormalTok{)}
\NormalTok{litter <{-}}\StringTok{ }\KeywordTok{read.csv}\NormalTok{(}\StringTok{"./Data/Processed/NEON\_NIWO\_Litter\_mass\_trap\_Processed.csv"}\NormalTok{)}
...
\begin{Highlighting}[]
\CommentTok{\#3}
\NormalTok{lake\_filter <{-}}\StringTok{ }
\StringTok{  }\NormalTok{lake }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{filter}\NormalTok{ ((lake}\OperatorTok{$}\NormalTok{daynum }\OperatorTok{>}\StringTok{ }\DecValTok{181}\NormalTok{) }\OperatorTok{\&}\StringTok{ }\NormalTok{(lake}\OperatorTok{$}\NormalTok{daynum }\OperatorTok{<=}\StringTok{ }\DecValTok{213}\NormalTok{)) }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{select}\NormalTok{(lakename, year4, daynum, depth, temperature\_C) }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{na.omit}\NormalTok{()}
  
\CommentTok{\#4}
\NormalTok{lakeAIC <{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ lake\_filter, temperature\_C }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{step}\NormalTok{(lakeAIC)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ year4 + daynum + depth, data = lake_filter)
## 
## Coefficients:
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lake\_model <{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ lake\_filter, temperature\_C }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{year4 }\OperatorTok{+}\StringTok{ }\NormalTok{daynum }\OperatorTok{+}\StringTok{ }\NormalTok{depth)}
\KeywordTok{summary}\NormalTok{(lake\_model)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: The three explanatory variables year, day number and lake depth
significantly predict lake temperatures (Multiple linear regression,
F(3,9972)=9457, p \textless{} 0.001). Our model accounts for 73.98\% of
the variance in observed temperatures.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{\#6}
\NormalTok{lake\_ancova <{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ lake\_filter, temperature\_C }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{depth }\OperatorTok{*}\StringTok{ }\NormalTok{lakename)}
\KeywordTok{summary}\NormalTok{(lake\_ancova)}
\end{Highlighting}
\end{Shaded}
...
## 
## Call:
## lm(formula = temperature_C ~ depth * lakename, data = lake_filter)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
...
\begin{quote}
Answer: Yes there is a significant interaction effect between depth and
lakename. All th interaction effects are significant with a p-value of
less than 0.05 except the interaction of depth and paul lake whose
p-value is 0.11. 78.55\% of variance in the temperature observations can
explained by the ANCOVA.
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{\#8}
\KeywordTok{ggplot}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ lake\_filter, }\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ depth, }\DataTypeTok{y=}\NormalTok{temperature\_C, }\DataTypeTok{color=}\NormalTok{lakename)) }\OperatorTok{+}
...
\StringTok{  }\KeywordTok{geom\_smooth}\NormalTok{ (}\DataTypeTok{method =} \StringTok{"lm"}\NormalTok{, }\DataTypeTok{se =} \OtherTok{FALSE}\NormalTok{, }\DataTypeTok{size =} \DecValTok{1}\NormalTok{, }\DataTypeTok{lty =} \DecValTok{2}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{labs}\NormalTok{ (}\DataTypeTok{x =} \StringTok{"Depth (m)"}\NormalTok{, }\DataTypeTok{y =} \KeywordTok{expression}\NormalTok{(}\StringTok{"Temperature ("}\OperatorTok{\textasciitilde{}}\NormalTok{degree}\OperatorTok{*}\StringTok{"C)"}\NormalTok{), }\DataTypeTok{color =} \StringTok{"Lakes"}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\KeywordTok{scale\_color\_manual}\NormalTok{ (}\DataTypeTok{values =} \KeywordTok{colorRampPalette}\NormalTok{(}\KeywordTok{brewer.pal}\NormalTok{(}\DecValTok{8}\NormalTok{, }\StringTok{"Set1"}\NormalTok{))(}\DecValTok{9}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#a.}
\NormalTok{litter\_mixed <{-}}\StringTok{ }\KeywordTok{lme}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ litter, dryMass }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{nlcdClass }\OperatorTok{*}\StringTok{ }\NormalTok{functionalGroup,}
                    \DataTypeTok{random =} \OperatorTok{\textasciitilde{}}\DecValTok{1}\OperatorTok{|}\NormalTok{plotID)}
\KeywordTok{summary}\NormalTok{(litter\_mixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#b.}
\KeywordTok{rsquared}\NormalTok{(litter\_mixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{quote}
Answer: The variance increased by 2.132 \% by adding the random effect
to the model
\end{quote}
...
\begin{Highlighting}[]
\CommentTok{\#c.}
\NormalTok{litter\_fixed <{-}}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{data =}\NormalTok{ litter, dryMass }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{nlcdClass }\OperatorTok{*}\StringTok{ }\NormalTok{functionalGroup)}
\KeywordTok{summary}\NormalTok{(litter\_fixed)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\CommentTok{\#d.}
\KeywordTok{anova}\NormalTok{(litter\_mixed, litter\_fixed)}
\end{Highlighting}
\end{Shaded}

\begin{verbatim}
##              Model df      AIC      BIC    logLik   Test  L.Ratio p-value
## litter_mixed     1 26 9038.575 9179.479 -4493.287                        
## litter_fixed     2 25 9058.088 9193.573 -4504.044 1 vs 2 21.51338  <.0001
\end{verbatim}
...
\begin{quote}
Answer: Our ANOVA returned a p-value less than 0.0001 meaning that both
the models have a significantly different fit. The mixed effects model
is a better model because it has a lower AIC and BIC value.
\end{quote}
