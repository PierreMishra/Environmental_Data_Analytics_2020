...
\begin{Highlighting}[]
\CommentTok{\# 2 }
\NormalTok{GaringerOzone <{-}}\StringTok{ }\KeywordTok{rbind}\NormalTok{(GaringerNC2010, GaringerNC2011,}
\NormalTok{                       GaringerNC2012, GaringerNC2013,}
\NormalTok{                       GaringerNC2014, GaringerNC2015,}
\NormalTok{                       GaringerNC2016, GaringerNC2017,}
\NormalTok{                       GaringerNC2018, GaringerNC2019)}
...
\CommentTok{\# 4}
\NormalTok{GaringerOzone\_wrangled <{-}}\StringTok{ }\KeywordTok{select}\NormalTok{(GaringerOzone, Date,}
\NormalTok{                                 Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration,}
\NormalTok{                                 DAILY\_AQI\_VALUE)}

\CommentTok{\# 5}
\NormalTok{dates <{-}}\StringTok{ }\KeywordTok{seq}\NormalTok{(}\KeywordTok{as.Date}\NormalTok{(}\StringTok{"2010{-}01{-}01"}\NormalTok{), }
            \KeywordTok{as.Date}\NormalTok{(}\StringTok{"2019{-}12{-}31"}\NormalTok{),}
            \DataTypeTok{by =} \StringTok{"day"}\NormalTok{)}
\NormalTok{dates <{-}}\StringTok{ }\KeywordTok{as.data.frame}\NormalTok{(dates)}

\CommentTok{\# 6}
\NormalTok{GaringerOzone <{-}}\StringTok{ }\KeywordTok{left\_join}\NormalTok{(dates, GaringerOzone\_wrangled,}
                           \DataTypeTok{by =} \KeywordTok{c}\NormalTok{(}\StringTok{"dates"}\NormalTok{ =}\StringTok{ "Date"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{yr =} \KeywordTok{year}\NormalTok{(dates),}
         \DataTypeTok{mo =} \KeywordTok{month}\NormalTok{(dates)) }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{group\_by}\NormalTok{(yr,mo) }\OperatorTok{\%>\%}
\StringTok{  }\KeywordTok{summarize}\NormalTok{(}\DataTypeTok{oz\_mean =} \KeywordTok{mean}\NormalTok{(Daily.Max.}\FloatTok{8.}\NormalTok{hour.Ozone.Concentration))}

\NormalTok{GaringerOzone.monthly}\OperatorTok{$}\NormalTok{dates <{-}}\StringTok{ }
\StringTok{  }\KeywordTok{as.Date}\NormalTok{(}\KeywordTok{paste}\NormalTok{(GaringerOzone.monthly}\OperatorTok{$}\NormalTok{yr, }
\NormalTok{                GaringerOzone.monthly}\OperatorTok{$}\NormalTok{mo,}
                \DecValTok{1}\NormalTok{, }\DataTypeTok{sep =} \StringTok{"{-}"}\NormalTok{))}

\CommentTok{\# 10}
\NormalTok{GaringerOzone.monthly.ts <{-}}\StringTok{ }\KeywordTok{ts}\NormalTok{(GaringerOzone.monthly}\OperatorTok{$}\NormalTok{oz\_mean, }
                               \DataTypeTok{frequency =} \DecValTok{12}\NormalTok{,}
                               \DataTypeTok{start =} \KeywordTok{c}\NormalTok{(}\DecValTok{2010}\NormalTok{,}\DecValTok{01}\NormalTok{,}\DecValTok{1}\NormalTok{),}
