...
```{r}
#1. 
four_seq <- seq(1,100,4) #Using the function "seq" to generate numbers from 1 to 100 and stepping the values by 4

#2. 
mean <- mean(four_multi)      # Calculating mean of the sequence generated
median <- median(four_multi)  # Calculating median of the sequence 

#3. 
mean > median # Comparing mean and median. It returns FALSE meaning mean is not greater than median
```
...
names(stud_info) <- c("Name", "Score", "Passed")
stud_info
```
