...
PMplot <- 
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year), color = Site.Name)) + ####WOW, shape as in circle and triangle
  geom_point()
print(PMplot) #### use print function for knitting in pdf

# Separate plot with facets
PMplot.faceted <-
  ggplot(EPAair, aes(x = Month, y = PM2.5, shape = as.factor(Year))) +
  geom_point() +
  facet_wrap(vars(Site.Name), nrow = 3) #### as soon as you hit the max number of columns, it will move to next row, ok...
#### nrows tells you number of rows in our facet plot, similaryl ncol
#### vars mean variables over which to make facets
...
PMplot.faceted2 <- 
  ggplot(subset(EPAair, Site.Name == "Clemmons Middle" | Site.Name == "Leggett" |
         ...
         aes(x = Month, y = PM2.5)) + 
  geom_point() +
  facet_grid(Site.Name ~ Year)  #### sie.name on the rigth side
#### to use include function in select
# PMplot.faceted2 <- 
#   ggplot(subset(EPAair, Site.Name %in% c("Clemmons Middlle", "Leggett", "Bryson City"), 
#          aes(x = Month, y = PM2.5)) + 
#   geom_point() +
...
PMplot.line <- 
  ggplot(subset(EPAair, Site.Name == "Leggett"), 
         aes(x = Date, y = PM2.5)) +
  geom_line() #### + geom_point() if you also wanna put (points)
print(PMplot.line)

#### if you have a gap in your time series, it will connect
...
ggplot (subset(PeterPaul.chem.nutrients,depth == 0), aes(x = daynum, y = temperature_C, color = year4)) +
  geom_point() + 
  facet_wrap(vars(lakename), nrow = 2) #### you can also put aes in geom_point
# face_grid (vars(lakename))
### they only sampled teice in print

#2. 
# Plot temperature by date. Color your points by depth.
# Change the size of your point to 0.5
ggplot (PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth), size = 0.5) +
  geom_point() 
...
lightvsDO <- 
  ...
  geom_point()
print(lightvsDO) #### those outliars are wrong values

# Adjust axes
lightvsDOfixed <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = irradianceWater, y = dissolvedOxygen)) +
  geom_point() +
  xlim(0, 250) + #### supply limits
  ylim(0, 20) 
print(lightvsDOfixed)
#### at lowlevel of irradiance water(depth) DO is so low, respiration is happening instead of photosynthesis
...
tempvsdepth <- 
  ...
  geom_point() +
  scale_y_reverse() #### it is just a practice to make it look like how things will look in real life
print(tempvsdepth)

NvsP <- #### looking at nitrogen vs phosphorus
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth)) +
  geom_point() +
  geom_smooth(method = lm, se = FALSE) + #### we are saying there is a linear relationshiip, se and remove the conidence interval shape
  geom_abline(aes(slope = 16, intercept = 0))
print(NvsP)
#### deeper depth has higher concentration of phos and nitrogen, that shaded thing is confidence inteval
...
Nutrientplot3 <-
  ...
  geom_boxplot(aes(color = nutrient)) # Why didn't we use "fill"? Because our cencentrations are low and it just looks dark, we used separate aesthetics for boxplot
print(Nutrientplot3)
...
Nutrientplot5 <-
  ggplot(PeterPaul.chem.nutrients.gathered, aes(x = lakename, y = concentration)) +
  geom_violin(aes(color = nutrient)) # when they are a lot of centered around zero, it looks wide
print(Nutrientplot5)
...
Nutrientplot6 <-
  ...
  geom_freqpoly(aes(x = tp_ug), color = "darkblue") + #### its good to specify aesthetics in different la
  geom_freqpoly(aes(x = nh34), color = "blue") +
  geom_freqpoly(aes(x = no23), color = "royalblue") +
  geom_freqpoly(aes(x = po4), color = "red") 
print(Nutrientplot6)

# Using a gathered dataset (more like wide format???)
Nutrientplot7 <-   
...
Nutrientplot6 <-
  ...
  geom_density_ridges(aes(fill = lakename), alpha = 0.5) # alpha is our transparency
print(Nutrientplot6)
