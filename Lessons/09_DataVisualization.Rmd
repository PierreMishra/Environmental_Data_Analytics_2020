...
O3plot4 <- ggplot(EPAair) +
  geom_point(aes(x = Date, y = Ozone)) +
  theme_classic()  #### this is her favorite
print(O3plot4)
```
...
```{r, fig.height = 3, fig.width = 4, warning = FALSE}
mytheme <- theme_classic(base_size = 14) +         #### ooooo you can create your own theme :D
...
O3plot5 <- ggplot(EPAair) +                  #### x and y marks became black instead of black
  geom_point(aes(x = Date, y = Ozone)) +
  mytheme
print(O3plot5)  
...
O3plot7 <- ggplot(EPAair, aes(x = Date, y = Ozone)) +
  geom_hline(yintercept = 50, lty = 2) +      #### hline is a horizontal value, line type (lty) 2 is dash line
  geom_hline(yintercept = 100, lty = 2) +     
  geom_point(alpha = 0.3, size = 1.5) +       ####ggplot always put thinks in layers, so she did geompoint AFTER hlline coz she wants it behind the line
  geom_text(x = as.Date("2020-01-01"), y = 45, label = "good", hjust = 1, fontface = "bold") + #### x to put it at 2020 jan 1, and y values
  ...
  scale_x_date(limits = as.Date(c("2018-01-01", "2019-12-31")), #### we just want our x axis to go from jan 2018 to jan 2020 
    date_breaks = "2 months", date_labels = "%b %y") + #### changin date labels, b is 3 letter months
  ylab(expression("O"[3]* " AQI Value")) + #### square bracket means subscript, star indicates I am finished making subscript
...
scales::show_col(colormap(colormap = colormaps$magma, nshades = 4))
...
NvsP2 <-
  ggplot(PeterPaul.chem.nutrients, aes(x = tp_ug, y = tn_ug, color = depth, shape = lakename)) +
  geom_point(alpha = 0.7, size = 2.5) + #ylab("TN (ug/l)") + xlab("TP (ug/l)") +
  labs(x = expression(paste("TP(", mu, "g/L)")), y = expression(paste("TP(", mu, "g/L)")),
  ...
  scale_color_viridis(option = "magma", direction = -1, end = 0.5) +   # direction is like light colors will represent low values and vice versa with -1, end chops lower 60 percent (if end = 0.4), you could also use "begin"
...
Nutrientplot <-
  ...
  geom_boxplot() +
  labs(x = "", y = expression(paste("Concentration(", mu, "g/L)")),
       color ="Nutrients") +  #### use something like that scale color discrete
