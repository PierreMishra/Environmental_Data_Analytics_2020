...
```{r check.pdf}
pdf(here("outputs", "pdf_test.pdf"), width = 11, height = 8.5) # for large datasets we can directly save it as pdf for plotting, coz its slow on the plots package
ggplot(data = cars) +
...
here("data", "tabular_data", "conterm_basinid.txt") #go to data folder, go to tabular_data folder etc...
# it is just get 
...
A coordinate reference systems defines how we convert (or we can say "project") a 3D object (in this case, the Earth) to 2D surface. We need coordintae refernce system because the shape of earth does not allow for its accurate represntaion on 2d space.
...
Maps package is also greaaaatttt for diffferent kinds of boundaries!!!!!! She strongly recommends it.
...
```{r read.in.some.data}
basins_nf_seplains_raw <- st_read(here("data", "spatial_data", "bas_nonref_SEPlains.shp")) # we are using st_read, notice we have a shapefile with epsg code and coordinates, we only need to read in the .shp file though we have 6 files in our spatial data folder
...
# add your code here
class(basins_nf_seplains_raw) #sf and dataframe, there could be two classes at once
...
# you can also use glimpse data from tibble(tidyverse) project
```
...
st_crs(gages_raw)
st_crs(southeast_state_bounds_raw) # for all of them i get "No EPSG code" but we want to have same coordinate reference systems or they are not gonna line up propoerly
...
When *projecting* a CRS, you're converting the spatial data from one CRS to another. For the sf package, transform (projecting) is used interchangeably with project.
...
my_epsg <- 5070 #they are both associated to the same reference system, so it is all good! Be careful regarding that

# setting projections
st_crs(basins_nf_seplains_raw) <- my_proj4 #this is what we are doing below in pipe function
...
What if you don't have a .shp file; instead, you have tabular data with spatial variables (longitude and latitude) in columns? You're in luck! There are ways to convert tabular data to sf objects.
...
gages_tabular_as_sf <- st_as_sf(gages_tabular, coords = c("LNG_GAGE", "LAT_GAGE"), crs = 4326, dim = "XY") # lat long so set CRS = WGS84, we are specifying refernce system that is why we are setting a reference system, if we already had an sf object we would just use st.transform to change the crs system

# check class and CRS
class(gages_tabular_as_sf)
head(gages_tabular_as_sf) # we dont have latitude and longitude but we have a geometry column
...
class(southeast_state_bounds_geometry) # its no longer a dataframe O_O
...
names(southeast_state_bounds)
names(va_state_bounds_geom) #notice it is null
class(southeast_state_bounds)
...
va_basins_nf_seplains <- basins_nf_seplains %>%
  st_intersection(va_state_bounds_geom) # basins is our base and
...
# bonus
# add your code here
va_area <- st_area(va_state_bounds_geom) # find the area of Virgnia
va_centroid <- st_centroid(va_state_bounds_geom) # find the centroid of the Virginia, we can use that as a spatial dataset that we can plot
va_gages_buffer_100m <- st_buffer(va_gages, dist = 100) # make a buffer around the stream gages in Virgina, distance is in unit of dataset (meters)
not_nc_gages <- st_difference(gages, va_state_bounds_geom) # find stream gages not in Virginia, gages that are outside of the cookie cutter
```
...
```{r basic.sf.plotting}
# basic plot, she put ggplot parantheiss emty coz she wanna add layers for geospatial stuff
pdf(here("outputs", "southeast_state_bounds.pdf"), width = 11, height = 8.5) #first she is plotting boundary, and send it to pdf coz it is faster than plotting it in plot window
ggplot() +
...
# checking virginia state boundary
pdf(here("outputs", "va_state_bounds.pdf"), width = 11, height = 8.5) #first she is plotting boundary, and send it to pdf coz it is faster than plotting it in plot window
ggplot() +
...
# add your code here
ggplot() +
  geom_sf(data = va_state_bounds_geom, color = "red") +
  #geom_sf(data = va_basins_nf_seplains, aes(fill = va_basins_nf_seplains$AREA)) +
  geom_sf(data = va_basins_nf_seplains, fill = "pink", alpha = 0.25) +#there are different leves of pinks because darker pinkareas mean that there are more than 1 watershaed overlaping that is giving it a darker color
  geom_sf(data = va_gages)
dev.off()
```
...
# get data bounding box, we want basemap that can show us more area than just NC
nc_bbox <- nc_state_bounds_geom %>%
  ...
  st_transform(4326) %>% # WGS84 (for lat and long) (we are changing it back to latitude and longitude)
  st_bbox() #stbbox will return coordinates of the corners
nc_bbox # check

# fix bounding box columns so they match what is needed for the ggmap::get_map() function, to pull basemap, we use output from ncbbox 
nc_bbox_fix <- c(left = nc_bbox[[1]], bottom = nc_bbox[[2]], right = nc_bbox[[3]], top = nc_bbox[[4]])

# check result
nc_bbox_fix

# get basemap, stamen is the name of the basemap
nc_basemap <- get_map(nc_bbox_fix, maptype = 'terrain-background', source = 'stamen', zoom = 8) # zoom varies from 3 to 21 (continental scale to building scale)
class(nc_basemap) # notice it is a rastor
st_crs(nc_basemap) # notice it is nothing because wach of the files just got loaded are a tile
...
ggmap(nc_basemap) + # notce instead of using ggplot, we are using ggmap
...
names(nc_gages)
# use "STAID"
nc_gages$STAID <- as.numeric(as.character(nc_gages$STAID))

# join the tabular data to nc_gages
nc_gages_climate <- nc_gages %>%
  #mutate(nc_gages, class = numeric()) to hange the type
