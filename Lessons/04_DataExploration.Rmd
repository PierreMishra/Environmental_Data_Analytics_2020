...
getwd() #whenevr you start working on a project, it is good ides to verufuy working directory
```
...
#### you can also set working directory from 'sessions' option (above)
#### write comment like the one written here on line 35
```
...
```{r, message = FALSE} 
#### she did `message = FALSE` so when you knit, it doesnt print this and prevent any knitting errors. NICE!
# Load package
library(tidyverse)
```
...
USGS.flow.data <- read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") #### '.' in beginning means go 1 folder in
#### '..' 2 dots in beginning mean go out 1 folder and then go in to what we are writing.
#### '~' also has some option. but kateri doesnt know so dont worry about it

# What happens if we don't assign a name to our imported dataset?
#read.csv("../Data/Raw/USGS_Site02085000_Flow_Raw.csv") #### DONT DO IT THIS WAY

# Another option is to choose with your browser
# read.csv(file.choose()) #### THIS IS ALSO NOT REPRODUCIBLE!

# To import .txt files, use read.table rather than read.csv
#read.table() #### This is what Luana (TSA professor) uses
...
> ANSWER: It is good to hav some consistent form of naming conventions. Return back to the readme files often.

```{r}
View(USGS.flow.data)
# Alternate option: click on data frame in Environment tab
#### to empty 'Environment' on the right, click on the broom looking icon.
...
# If you would want to rename only 1 or a few of the column names, use this below:
colnames(USGS.flow.data)[1] <- "agency"

str(USGS.flow.data) #### structure of our dataframes. it's like a little summary of datatypes
#### whatever is NA was an empty cell in real excel file
dim(USGS.flow.data)
length(USGS.flow.data) #### number of columns, you can use 'nrow' for number of rows

head(USGS.flow.data)
head(USGS.flow.data, 10)
tail(USGS.flow.data, 5)
USGS.flow.data[30000:30005, c(3, 8, 14)] #### thats a way to do in matrix subsetting

class(USGS.flow.data$datetime) #### it is factor, but we know it is a date
...
> Answer: if it is a numeric value and blank it will be NA, if it is factor or char it will be blank
...
```{r}
USGS.flow.data.complete <- na.omit(USGS.flow.data) #### it takes out any row that contains NA just once
dim(USGS.flow.data)
...
In some cases when dates are provided as integers, you may need to provide an origin for your dates. Beware: the "origin" date for Excel (Windows), Excel (Mac), R, and MATLAB all have different origin dates. Google this if it comes up. #### jan 1 1970 in R
...
format(today, format = "%d") #### because it is in date format, R can extract all the info for that day, like week

USGS.flow.data$datetime <- as.Date(USGS.flow.data$datetime, format = "%m/%d/%y") #### format is watever format is in your data
#### Now if you look at your data, it is formatted in the global format of year-month-date
```
...
create.early.dates <- (function(d) {
       paste0(ifelse(d > 191231,"19","20"),d) #### if date is greater than that time, we will put 
       })
#
