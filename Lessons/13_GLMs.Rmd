...
summary(TNancova.main) # intercept will be paul lake at zero meters depth, it will have tn concentration of 284, peter lake is 66 more than paul lake and significantly higher. At each 1 ubnit increase in depth, TN increase by 68

# interaction effects (we might have different intercept for each lake but we might also have diffrent beta for each lake)
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth)
summary(TNancova.interaction) #intercept is paul lake at depth of zero, if the lake is paul lake each increase in depth will be an increase in 90 tn, and peterlake at that depth would be -35 of what paul lake was. We have explained 42 percent of the variation by our interaction effect

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) + 
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) + # this helps to make a line
...
All these exampless of models we have talked about had fixed effects
...
```{r}
# we might be interensted not in specific lakes but more geenrally how lakes have 

NTL.summertemp <- 
  NTL.chem %>%
  select(lakename:temperature_C) %>%
  #filter for Julian days in July and surface measurements
  filter(daynum > 181 & daynum < 213 & depth == 0 ) %>%  #selecting from month of July
  #code won't work if there are NAs
  na.exclude() # excluding NA's 

NTLtemps <-
ggplot(NTL.summertemp, aes(x = sampledate, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d()
print(NTLtemps)     # take this variability into characterize
```
...
```{r}
TempTest.mixed <- lme(data = NTL.summertemp, # lme is linear mixed effects, we wanna know hoe temperature varies by year, so year as our fixed effect
                     temperature_C ~ year4, 
                     random = ~1|lakename) #since we are just grouping by lakename we used 1 (random fact of our model). We might be interested in knowing what exactly the values for year that is why it is a fixed value, random effect typically shd be category
summary(TempTest.mixed) # it rounds p-value, randome effect doesnt have p-value  because we are nt interested in measured effect of random effect. We are just interested in incoporating it in our model, each year the temperature of lake is increased by 0.06 degree
rsquared(TempTest.mixed) # ths gives us what the R2 would be if we just had fixed effect and then with random effect  (conditional is for random effect, marginal is for fixed effect), 

# Compare the random effects model with the fixed effects model
TempTest.fixed <- lm(data = NTL.summertemp, # you could still use lm and still do ANOVA.,change lm to gls but lm doesnt give same R2 as it gave in above Rsquared fucntions  for  marginal (fixed)
                      temperature_C ~ year4)
summary(TempTest.fixed) # fixed effects model

anova(TempTest.mixed, TempTest.fixed) # anova on fixed and mixed effects model to see if there are significant differences between the model, it also gives p-value, which means it is different
...
ggplot(NTL.summertemp, aes(x = year4, y = temperature_C, color = lakename)) +
  geom_point() +
  scale_color_viridis_d() +
  geom_abline(intercept = -97.72, slope = 0.06) + # we used this intercet and sloope from mixed effects model
  # make it look better
  labs(x = "", y = expression("Temperature " ( degree~C)), color = "") +
  theme(legend.spacing.x = unit(0, "cm"))
print(NTL.tempmodel)

#line of best fit associated with fixed effects and mixed effects model (comparison)
